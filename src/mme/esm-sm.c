...
printf("OGS_NAS_EPS_ESM_INFORMATION_RESPONSE\n");
        ...
        case OGS_NAS_EPS_ACTIVATE_DEDICATED_EPS_BEARER_CONTEXT_ACCEPT:
            printf("Activate dedicated EPS bearer context accept\n");
            printf("    IMSI[%s] PTI[%d] EBI[%d]\n",
                    mme_ue->imsi_bcd, sess->pti, bearer->ebi);
            /* Check if Initial Context Setup Response or 
             *          E-RAB Setup Response is received */
...
void esm_state_active(ogs_fsm_t *s, mme_event_t *e)
{
    ...
    case MME_EVENT_ESM_MESSAGE:
        ...
        case OGS_NAS_EPS_PDN_CONNECTIVITY_REQUEST:
            ...
            ogs_debug("    IMSI[%s] PTI[%d] EBI[%d]",
                    mme_ue->imsi_bcd, sess->pti, bearer->ebi);
            // printf("[esm_state_active] message->esm.pdn_connectivity_request.access_point_name = '%s'\n", message->esm.pdn_connectivity_request.access_point_name.apn);
// 
            // is emergency make the access_point_name same as sessions?
            printf("mme_ue->session[0].name : '%s'\n", mme_ue->session[0].name); 
            printf("mme_ue->session[2].name : '%s'\n", mme_ue->session[2].name); 
            // if (OGS_NAS_EPS_REQUEST_TYPE_EMERGENCY == message->esm.pdn_connectivity_request.request_type.value) {
            //     if (!strcmp("internet", mme_ue->session[0].name)) {
...
void esm_state_pdn_will_disconnect(ogs_fsm_t *s, mme_event_t *e)
{
    ...
    case MME_EVENT_ESM_MESSAGE:
        ...
        case OGS_NAS_EPS_PDN_CONNECTIVITY_REQUEST:
            ...
            ogs_debug("    IMSI[%s] PTI[%d] EBI[%d]",
                    mme_ue->imsi_bcd, sess->pti, bearer->ebi);
            printf("[esm_state_pdn_will_disconnect] message->esm.pdn_connectivity_request.access_point_name = '%s'\n", message->esm.pdn_connectivity_request.access_point_name.apn);
            rv = esm_handle_pdn_connectivity_request(
                    bearer, &message->esm.pdn_connectivity_request,
                    e->create_action);
            if (rv != OGS_OK) {
