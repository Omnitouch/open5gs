...
static void _gtpv1_u_recv_cb(short when, ogs_socket_t fd, void *data)
{
    ...
    //         if (is_uplink) {
    //             mme_metrics_inst_global_add(MME_METR_GLOB_GAUGE_GTP_INDATAOCTS1U, gtpu_data_length);
    //             mme_metrics_inst_global_inc(MME_METR_GLOB_GAUGE_GTP_INDATAPACKETS1U);
    //         } else {
    //             mme_metrics_inst_global_add(MME_METR_GLOB_GAUGE_GTP_OUTDATAOCTS1U, gtpu_data_length);
    //             mme_metrics_inst_global_inc(MME_METR_GLOB_GAUGE_GTP_OUTDATAPACKETS1U);
    //         }
    ...
    //             ogs_assert(far->sess);
    //             sess = MME_SESS(far->sess);
    //             ogs_assert(sess);

    //             ogs_assert(OGS_OK ==
    //                 mme_pfcp_send_session_report_request(sess, &report));
    //         }
    //     } else {
    ...
    //             mme_sess_urr_acc_add(sess, pdr->urr[i], gtpu_data_length, is_uplink);

    //         break;
    //     case OGS_PFCP_OBJ_SESS_TYPE:
    //         /* MME does not use SESS TYPE */
    //         ogs_assert_if_reached();
...
void mme_gtp1_final(void)
{
    ogs_pkbuf_pool_destroy(packet_pool);
}

int mme_gtp1_open(void)
{
